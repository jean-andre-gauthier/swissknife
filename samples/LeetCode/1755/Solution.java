...
public class Solution {
  public static void main(String[] args) {
    out.println(minAbsDifference(in.nextInts(in.nextInt()), in.nextInt()));
  }
  ...
  public static int minAbsDifference(int[] nums, int goal) {
      ...
      // out.println(goal);
      int n=nums.length;
      long[]sl=sums(nums,0,n/2);
      long[]sr=sums(nums,n/2,n);
      Arrays.sort(sr);
      // out.println(sl);
      // out.println(sr);
      long res=Long.MAX_VALUE;
      for(int sli=0;sli<sl.length;++sli){
        long target=goal-sl[sli];
        int idx=Arrays.binarySearch(sr,target);
        if(idx<0){
          long sr1=sr[Math.min(-idx-1,sr.length-1)];
          long sr2=sr[Math.max(-idx-2,0)];
          res=Math.min(res,Math.min(Math.abs(sl[sli]+sr1-goal),Math.abs(sl[sli]+sr2-goal)));
        }else{
          // out.println(sli+" "+idx+" "+target);
          return 0;
        }
      }
      return (int)res;
  ...
  static long[]sums(int[]nums,int l,int r){
    int imax=(int)Math.pow(2,r-l);
    long[]res=new long[imax];
    int resi=0;
