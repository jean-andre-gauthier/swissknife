...
public class Solution {
  public static void main(String[] args) {
    out.println(longestSubstring(in.next(),in.nextInt()));
  }
  ...
  public static int longestSubstring(String s, int k, int nct) {
    int res=0;
    int l=0;
    int r=0;
    int nc=0;
    ...
    while(r<s.length()){
      if(freqs[s.charAt(r)-'a']==0)nc++;
      freqs[s.charAt(r)-'a']++;
      if(freqs[s.charAt(r)-'a']==k)nck++;
      r++;
      while(nc>nct){
        if(freqs[s.charAt(l)-'a']==k)nck--;
        freqs[s.charAt(l)-'a']--;
        if(freqs[s.charAt(l)-'a']==0)nc--;
        l++;
      }
      if(nc==nck&&nc==nct)res=Math.max(res,r-l);
    }
    return res;
  }
