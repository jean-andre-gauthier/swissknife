...
public class Solution {
  ...
  public static int[][] updateMatrix(int[][] mat) {
      ...
      for(int j=0;j<n;++j){
        for(int i=0;i<m;++i){
          if(mat[i][j]!=0){
            if(i==0&&j==0)mat[i][j]=n+m;
            else if(j==0)mat[i][j]=mat[i-1][j]+1;
            else if(i==0)mat[i][j]=mat[i][j-1]+1;
            else mat[i][j]=Math.min(mat[i-1][j],mat[i][j-1])+1;
          }
        }
      }
      for(int j=n-1;j>=0;--j){
        for(int i=m-1;i>=0;--i){
          ...
          else if(j==n-1)mat[i][j]=Math.min(mat[i][j],mat[i+1][j]+1);
          else if(i==m-1)mat[i][j]=Math.min(mat[i][j],mat[i][j+1]+1);
          else mat[i][j]=Math.min(mat[i][j],Math.min(mat[i+1][j],mat[i][j+1])+1);
        }
      }
      return mat;
  }
