...
public class Solution {
  public static void main(String[] args) {
    ...
    for(int i=0;i<n;++i){
      intervals[i]=in.nextInts(2);
    }
    int[]newInterval=in.nextInts(2);
    out.println(insert(intervals,newInterval));
  }

  public static int[][] insert(int[][] intervals, int[] newInterval) {
      int sm=0;
      int n=intervals.length;
      while(sm<n&&intervals[sm][1]<newInterval[0])sm++;
      int em=sm;
      while(em<n&&intervals[em][0]<=newInterval[1])em++;
      int nm=em-sm;
      List<int[]>res=new ArrayList<>();
      for(int i=0;i<n;++i){
        if(i<sm||i>sm)res.add(new int[]{intervals[i][0],intervals[i][1]});
        else
          if(nm>0){
            res.add(new int[]{Math.min(intervals[sm][0],newInterval[0]),Math.max(intervals[em-1][1],newInterval[1])});
            i+=nm-1;
          }
          else{
