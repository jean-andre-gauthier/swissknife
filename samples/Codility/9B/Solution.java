...
public class Solution {
  public static void main(String[] args) {
    out.println(solution(in.nextInts(in.nextInt())));
  }
  ...
  public static int solution(int[] A) {
    ...
    for(int i=2;i<n;++i){
      maxsumend[i]=Math.max(maxsumend[i-1]+A[i-1],A[i-1]);
    }
    // out.println(maxsumend);
    int[]maxsumstart=new int[n];
    for(int i=n-3;i>=0;--i){
      maxsumstart[i]=Math.max(maxsumstart[i+1]+A[i+1],A[i+1]);
    }
    // out.println(maxsumstart);
    int max=0;
    for(int i=1;i<n-1;++i){
      max=Math.max(max,Math.max(maxsumend[i],0)+Math.max(maxsumstart[i],0));
    }
    return max;
  }
