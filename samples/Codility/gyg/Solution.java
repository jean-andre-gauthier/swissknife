...
public class Solution {
  ...
  public static String solution(String S) {
      ...
      for (int i=0;i<lines.size();++i){
        ...
        out.println(entries);
        String[] nameExtension=entries[0].split(".");
        LocalDateTime datetime = LocalDateTime.parse(entries[1], datetimeFormatter);
        LineEntry lineEntry = new LineEntry(nameExtension[0], nameExtension[1], entries[1], datetime, i, "");
        lineEntries.add(lineEntry);
      }
      Map<String,List<LineEntry>> cityToEntries=new HashMap<>();
      for (int i=0;i<lineEntries.size();++i){
        LineEntry lineEntry=lineEntries.get(i);
        if(cityToEntries.containsKey(lineEntry.city)) {
          cityToEntries.get(lineEntry.city).add(lineEntry);
        }else{
          cityToEntries.put(lineEntry.city, Arrays.asList(lineEntry));
        }
      }
      return "";
  }
  ...
  static class LineEntry {
    ...
    public LineEntry(String name, String extension, String city, LocalDateTime datetime, int index, String newname) {
      this.name = name;
