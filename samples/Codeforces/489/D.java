...
public class D {
    public static void main(String[] args) {
        ...
        List<Map<Integer,Integer>>gps=new ArrayList<>();
        for(int i=0;i<=n;++i){
            gps.add(new HashMap<>());
        }
        for(int a=1;a<=n;++a){
            for(Integer b:adj.get(a)){
                for(Integer c:adj.get(b)){
                    if(a!=c){
                        gps.get(c).put(a, gps.get(c).getOrDefault(a,0)+1);
                    }
                }
            }
        }
        ...
        for(int i=1;i<=n;++i){
//            out.println("c "+i);
            for(Map.Entry<Integer,Integer> es:gps.get(i).entrySet()){
//                out.println("a "+es.getKey());
//                out.print("bs ");
//                out.println(es.getValue(),false);
                int nps=es.getValue();
                res+=nps*(nps-1)/2;
            }
        }
