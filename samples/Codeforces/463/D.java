...
//        out.println(psi);
        ...
        for(int i=1;i<=n;++i){
            for(int j=1;j<=n;++j){
                boolean jAfterI=true;
                for(int ki=0;ki<k;++ki){
                    jAfterI&=(psi[ki][i]<psi[ki][j]);
                }
                if(jAfterI){
//                    out.println("i: "+i+" j: "+j);
                    nodes[i].next.add(nodes[j]);
                    nodes[j].prev.add(nodes[i]);
                }
            }
        }
        ...
        int res=1;
        for(Node curr:sorted){
            int length=1;
            for(Node next:curr.next){
                length=Math.max(length,next.length+1);
            }
            curr.length=length;
            res=Math.max(res,curr.length);
//            out.println(curr.i+" "+curr.length);
        }
        out.println(res);
    }

    static void dfs(Node curr,ArrayList<Node>sorted){
        if(!curr.visited){
            for(Node next:curr.next){
                dfs(next,sorted);
            }
