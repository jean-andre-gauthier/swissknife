...
public class D {
    public static void main(String[] args) {
        ...
        char[][]c=new char[50][];
        boolean[][]r=new boolean[50][50];
        int[][]b=new int[2500][2];
        int bi=0;
        int[][]g=new int[2500][2];
        int gi=0;
        for(int i=0;i<t;++i){
            int n=in.nextInt();
            int m=in.nextInt();
            ...
            for(int j=0;j<bi;++j){
                if(b[j][0]>0){
                    c[b[j][0]-1][b[j][1]]=c[b[j][0]-1][b[j][1]]=='.'?'#':c[b[j][0]-1][b[j][1]];
                }
                if(b[j][0]<n-1){
                    c[b[j][0]+1][b[j][1]]=c[b[j][0]+1][b[j][1]]=='.'?'#':c[b[j][0]+1][b[j][1]];
                }
                if(b[j][1]>0){
                    c[b[j][0]][b[j][1]-1]=c[b[j][0]][b[j][1]-1]=='.'?'#':c[b[j][0]][b[j][1]-1];
                }
                if(b[j][1]<m-1){
                    c[b[j][0]][b[j][1]+1]=c[b[j][0]][b[j][1]+1]=='.'?'#':c[b[j][0]][b[j][1]+1];
                }
...
//            out.println("--------");
    ...
    static void dfs(int l,int c,char[][]cs,boolean[][]r,int n,int m){
        r[l][c]=true;
        if(l>0&&!r[l-1][c]&&cs[l-1][c]!='#'){
            dfs(l-1,c,cs,r,n,m);
        }
        if(l<n-1&&!r[l+1][c]&&cs[l+1][c]!='#'){
            dfs(l+1,c,cs,r,n,m);
        }
        if(c>0&&!r[l][c-1]&&cs[l][c-1]!='#'){
            dfs(l,c-1,cs,r,n,m);
        }
        if(c<m-1&&!r[l][c+1]&&cs[l][c+1]!='#'){
            dfs(l,c+1,cs,r,n,m);
        }
    }
