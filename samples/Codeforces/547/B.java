...
//        out.println(a);
        ...
        lq.addFirst(0);
        for(int i=1;i<n;++i){
            while(lq.peekFirst()!=null&&a[lq.getFirst()]>=a[i]){
                lq.removeFirst();
            }
            l[i]=lq.peekFirst()==null?0:lq.getFirst()+1;
            lq.addFirst(i);
        }
//        out.println(l,false);
        int[]r=new int[n];
        ...
        for(int i=n-1;i>=0;--i){
            while(rq.peekFirst()!=null&&a[rq.getFirst()]>=a[i]){
                rq.removeFirst();
            }
            r[i]=rq.peekFirst()==null?n-1:rq.getFirst()-1;
            rq.addFirst(i);
        }
//        out.println(r,false);
        int[]lr=new int[n];
...
//        out.println(lr,false);
        long[]ans=new long[n];
        for(int i=0;i<n;++i){
            ans[lr[i]-1]=Math.max(ans[lr[i]-1], a[i]);
        }
        for(int i=n-2;i>=0;--i){
            ans[i]=Math.max(ans[i], ans[i+1]);
        }
        out.println(ans,false);
    }
