...
//        out.println(t, false);
    ...
    static boolean build(int[] t, int[] a, int ti, int al, int ar) {
      if (al == ar) {
        t[ti] = a[al];
        return true;
      }
        ...
        int am = (al + ar) / 2;
        boolean orl = build(t, a, ti*2, al, am);
        boolean orr = build(t, a, ti*2+1, am+1, ar);
        if (orl && orr) {
    ...
    static boolean update(int[] t, int ti, int al, int ar, int p, int b) {
      if (al == ar) {
        t[ti] = b;
        return true;
      } else {
        ...
        boolean or = true;
        if (p <= am) {
          or = update(t, 2*ti, al, am, p, b);
        } else {
          or = update(t, 2*ti+1, am+1, ar, p, b);
        }
        if (or) {
