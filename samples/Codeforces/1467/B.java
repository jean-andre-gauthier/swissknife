...
public class B {
    public static void main(String[] args) {
        ...
        for(int i=0;i<t;++i){
            ...
            for(int j=1;j<n-1;++j){
                ...
                nhv+=hvj?1:0;
                if(hvj){
                    int s1=(hv(a,j-1)?1:0)+(hv(a,j)?1:0)+(hv(a,j+1)?1:0);
                    int aj=a[j];
                    a[j]=a[j-1];
                    int s2=(hv(a,j-1)?1:0)+(hv(a,j)?1:0)+(hv(a,j+1)?1:0);
                    a[j]=a[j+1];
                    int s3=(hv(a,j-1)?1:0)+(hv(a,j)?1:0)+(hv(a,j+1)?1:0);
                    a[j]=aj;
                    resi=Math.max(resi,Math.max(s1-s2,s1-s3));
                }
            }
            res[i]=nhv-resi;
        }
    ...
    static boolean hv(int[]a,int j){
        return j!=0&&j!=a.length-1&&((a[j-1]<a[j]&&a[j+1]<a[j])||(a[j-1]>a[j]&&a[j+1]>a[j]));
    }
