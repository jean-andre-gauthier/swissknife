...
public class D {
    public static void main(String[] args) {
        ...
        Map<Integer,Integer>notToReplace=new HashMap<>();
        for(int i=0;i<n;++i){
            toReplace.put(a[i],toReplace.getOrDefault(a[i],0)+1);
            notToReplace.put(a[i],1);
        }
        ...
        for(Map.Entry<Integer,Integer>e:toReplace.entrySet()){
            e.setValue(e.getValue()-1);
        }
        toReplace.values().removeAll(Arrays.asList(0));
        int ri=0;
        for(int i=0;i<n;++i){
//            out.println(a[i]);
//            out.println(toReplace);
//            out.println(notToReplace);
            if((toReplace.containsKey(a[i])&&notToReplace.containsKey(a[i])&&replacements.get(ri)>a[i])
                || (!toReplace.containsKey(a[i])&&notToReplace.containsKey(a[i]))){
                    notToReplace.remove(a[i]);
            }else{
                toReplace.put(a[i],toReplace.get(a[i])-1);
                toReplace.remove(a[i],0);
                a[i]=replacements.get(ri);
                ri++;
            }
        }
        out.println(replacements.size());
        out.println(a);
    }
